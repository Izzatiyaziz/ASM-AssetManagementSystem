...
<?php
    ...
    if(isset($_POST['admin_login']))
    {
      $a_code=$_POST['a_code'];
      $a_pwd=($_POST['a_pwd']);//
      $stmt=$mysqli->prepare("SELECT a_code, a_pwd, a_id FROM db_admin WHERE a_code=? and a_pwd=? ");//sql to log in user
      $stmt->bind_param('ss',$a_code,$a_pwd);//bind fetched parameters
      $stmt->execute();//execute bind
      $stmt -> bind_result($a_code,$a_pwd,$a_id);//bind result
      $rs=$stmt->fetch();
...
<style>
  .custom-bg-blue {
    background-color: #5E1916; /* Change this color to the shade of blue you prefer */
...
<head>
  ...
  <title>Nihoma AMS - Admin Login</title>
        ...
        <form method="POST">
          <div class="form-group">
            <div class="form-label-group">
              <input type="code" id="inputCode" name="a_code" class="form-control" placeholder="Code" required="required" autofocus="autofocus">
              <label for="inputCode">Id code</label>
            </div>
          </div>
          <div class="form-group">
            <div class="form-label-group">
