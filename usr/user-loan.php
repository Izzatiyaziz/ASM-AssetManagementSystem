<?php
  ...
  $aid=$_SESSION['u_id'];
   //Add USer
   if(isset($_POST['pickup_assets']))
   {
           $p_name=$_POST['p_name'];
           $p_datetime=$_POST['p_datetime'];
           $p_pic= $_FILES["p_pic"]["name"];
           move_uploaded_file($_FILES["p_pic"]["tmp_name"],"../vendor/img/".$_FILES["p_pic"]["name"]);
           $query="insert into db_pickup (p_name, p_datetime,p_pic) values(?,?,?)";
           $stmt = $mysqli->prepare($query);
           $rc=$stmt->bind_param('sss', $p_name, $p_datetime,$p_pic );
           $stmt->execute();
               if($stmt)
               {
    ...
    <div id="content-wrapper">
      ...
      <?php if(isset($succ)) {?>
        ...
        <div class="form-group">
                ...
                <input type="text" required readonly class="form-control" value="<?php echo $row->u_name;?>" id="exampleInputEmail1" name="p_name">
            </div>
        <?php }?>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="file" id="exampleInputEmail1" name="p_pic">
        </div>
        <button type="submit" name="pickup_assets" class="btn btn-success">Submit</button>
       </form>
       <!--End Form-->
      </div>
    </div>
